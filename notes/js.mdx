...
`var` hoist with undefined value
...
`Promise.all` aborts on the first rejection, whereas `Promise.allSettled` always waits for every operation to finish, regardless of success or failure.

## npm ci

Automated environments (CI/CD, Docker, Production), delete the folder and starts fresh, never update package.json, can throw an error and fail, can't use for install a single package