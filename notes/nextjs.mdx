...
date: "6-7-2025"
tags: ["Next.js"]
image: "/images/notes/next.js.png"
---
...
2. export const dynamic = 'force-dynamic', will effect all requests in the route
...
1. use Dynamic Function like 'cookies()' will skip Full Route and still use Data cache
2. use 'revalidate = 0' will skip the Full Route & Data Cached, mean component will rerender on each request
3. Opt-out the Data Cach by make `fetch(`https://...`, { cache: 'no-store' })`
4. when page set 'revalidate = 0', run every revalidate tag will reload this page
...
1. Calling route.refresh()
2. Revalidate Data Cache with on-demand revalidate
3. Using cookies prevent route use cookie from old data
